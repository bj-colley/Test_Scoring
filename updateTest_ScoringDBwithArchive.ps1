...
#variables:
$importPath = "\\ad.wiu.edu\wiu\Admin Data\heatupdate\"
...
$table_BAK = "Users_BAK"
$today = Get-Date
$month = Get-Date -UFormat %m #needed in order to get the 01, 02,...,09 values correct
...
#If today is the last day of the month (ie: tomorrow is the first...)
#delete the cooresponding month from last year, then move all files into folder
if($today.AddDays(1).day -eq "01"){
	del ($importPath+$month+"\*")
	move ($importPath+"heatuser"+$today.Year+$month+"*") ($importPath+$month)
}